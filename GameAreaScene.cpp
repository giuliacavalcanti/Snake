...
void GameAreaScene::startGame() {
    ...
    this->previousVector = RIGHT;

    this->bonusTiks = 0;
    this->isBonusVisible = false;

    this->bonusItem = this->addPixmap(QPixmap(":/images/images/Bonus.png"));
    this->bonusItem->setData(0, "Bonus");
    this->bonusItem->setVisible(false);
    this->bonusVisibleTimeLeft = 20;

    this->foodItem = this->addPixmap(QPixmap(":/images/images/Food.jpg"));
    this->foodItem->setData(0, "Food");
...
void GameAreaScene::addGameElement(GameElement gameElement) {
    ...
    while (ok==false) {
        ...
        if (gameElement == BONUS || gameElement == FOOD) {
            rect.setX((rand()%(int)(this->width())));
            rect.setY((rand()%(int)(this->height()/this->snakeHeight)));
            rect.setY(rect.y()*this->snakeHeight);
        } else if (gameElement == SNAKE) {
